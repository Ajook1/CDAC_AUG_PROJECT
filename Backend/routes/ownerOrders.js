...
router.get('/owner/orders', authOwner, (req, res) => {
...
/* Fetching details of a specific order */
router.get('/owner/orders/:order_id', authOwner, (req, res) => {
...
/* Modify Order Status like  (Confirmed / Shipped / Delivered) */
router.patch('/owner/orders/:order_id/status', authOwner, (req, res) => {

    const { order_id } = req.params;
    const { status } = req.body;

    if (!status) {
        return res.send(createResult('Order status is required'));
    }
    ...
    pool.query(sql, [status, req.user.user_id, order_id], (err) => {
        if (err) return res.send(createResult(err));
...
/* Order Cancel  */
router.patch('/owner/orders/:order_id/cancel', authOwner, (req, res) => {
